...
- **Delete event** (under More event tools) and **Delete client and records** (on the client profile) are server commands for clearing test or unwanted records. An event takes its recipes, quotations, payments, deliveries, hire bookings and tasks with it; the client and their enquiries stay. A client takes all of their events plus their enquiries, notes and routine orders. Stock still reserved for deleted orders is released back to Inventory, as when an order is cancelled, and nothing is recorded as wastage; stock already used in production stays used. The confirmation lists what will be removed, and the audit history keeps a copy.
