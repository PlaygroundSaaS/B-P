...
export async function POST(request:Request){
 try{
  ...
  const {data,error}=await db.from('studio_reviews').update({...review,published:true,submitted_at:new Date().toISOString()}).eq('id',token.id).eq('workspace_key',STUDIO_WORKSPACE).eq('token_hash',createHash('sha256').update(token.secret).digest('hex')).is('submitted_at',null).is('revoked_at',null).gt('expires_at',new Date().toISOString()).select('id');
  ...
  return json({message:'Thank you. Your review is now on our website.'});
