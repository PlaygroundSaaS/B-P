...
import { stockSummary, stockValue } from '@/lib/studio-commands';
...
import FlowerNamePicker, { FlowerThumbnail, findCatalogueFlower } from './flower-name-picker';
export default function InventoryDetails({ data, save, command }: { data: StudioData; save: SaveData; command: RunCommand }) {
  ...
  async function addFlower(event: FormEvent<HTMLFormElement>) {
    ...
    const reference = findCatalogueFlower(name);
    const flower: InventoryItem = { id: newId(), name, colour: reference?.colour || '', ...(reference ? { supplier: 'Flowervision Southampton' } : {}), stemsPurchased: quantity, stemsRemaining: quantity, costPerStem: cost, stockUnit: 'stem', category: 'Flowers', purchaseDate: day() };
    setBusy(true); setQuickError('');
    try {
      if (await save({ ...data, inventory: [flower, ...data.inventory] }, 'Flower added — ready to use in recipes.')) { form.reset(); setFlowerName(''); }
    } catch { setQuickError('The flower could not be saved. Your entries are still here; please try again.'); }
    finally { setBusy(false); }
  }
  async function deleteStock(stock: InventoryItem) {
    if (busy) return;
    if (stockSummary(data, stock.id).reserved > 0) { setStockError(`${stock.name} is reserved for purchased work. Amend or cancel that recipe before deleting it.`); return; }
    if (!confirm(`Delete ${stock.name} from your stock? It will not be recorded as wastage and no other stock figures will change. This cannot be undone.`)) return;
    setBusy(true); setStockError('');
    try { await command({ type: 'deleteStock', inventoryId: stock.id }); } finally { setBusy(false); }
  }
  return <section className="ops-panel">
    ...
    <form onSubmit={addFlower} className={styles.quickForm}>
      <fieldset disabled={busy}>
        <FlowerNamePicker value={flowerName} onChange={setFlowerName} />
    ...
    {stockError && <p role="alert" className="ops-warning">{stockError}</p>}
    <div className="ops-table-scroll"><table><thead><tr><th>Flower / item</th><th>Units bought</th><th>Available</th><th>Cost per stem / unit</th><th>Stock value</th><th /></tr></thead><tbody>{data.inventory.filter(i => `${i.name} ${i.colour || ''} ${i.supplier || ''} ${i.location || ''}`.toLowerCase().includes(search.toLowerCase())).map(i => { const stock = stockSummary(data, i.id); const value = stockValue(data, i.id); return <tr key={i.id}><td><span className={styles.stockName}><FlowerThumbnail key={i.name} flower={findCatalogueFlower(i.name)} /><span>{i.name}{i.colour && <><br /><small>{i.colour}</small></>}</span></span></td><td>{i.stemsPurchased}</td><td>{stock.available}{stock.reserved > 0 && <><br /><small>{stock.reserved} reserved</small></>}</td><td>£{i.costPerStem.toFixed(2)}{i.stockUnit && i.stockUnit !== 'stem' && <small> / {i.stockUnit}</small>}</td><td>{money(value.value)}{value.reserved > 0 && <><br /><small>incl. {money(value.reserved)} reserved</small></>}</td><td><span className={styles.rowActions}><button type="button" disabled={busy} onClick={() => { setItem(structuredClone(i)); setWastePhoto([]); }}>Edit / wastage</button><button type="button" className={styles.deleteButton} disabled={busy} aria-label={`Delete ${i.name}`} onClick={() => void deleteStock(i)}>Delete</button></span></td></tr>; })}</tbody></table>{!data.inventory.length && <p>Add your first flower above to start building recipes.</p>}</div>
