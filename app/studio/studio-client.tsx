...
export default function StudioClient() {
  ...
  const [tab, setTabValue] = useState<'dashboard' | 'inventory' | 'calculator' | 'jobs' | 'clients' | 'settings' | 'insights' | 'catalogue' | 'production' | 'suppliers' | 'deliveries' | 'hire' | 'payments' | 'reports' | 'calendar' | 'leads' | 'tasks' | 'recurring' | 'assistant' | 'activity'>('dashboard');
  ...
  const setTab = (section: typeof tab) => { if (section === tab || !canNavigate()) return; setTabValue(section); setEventDraft(null); setSelectedWeddingPlanId(null); setWeddingBuildDraft(null); updateLocation(screen, section); window.scrollTo(0, 0); };
  ...
  useEffect(() => {
    const restore = () => {
      ...
      if (space === 'business') {
        ...
        if (['dashboard', 'inventory', 'calculator', 'jobs', 'clients', 'settings', 'insights', 'catalogue', 'production', 'suppliers', 'deliveries', 'hire', 'payments', 'reports', 'calendar', 'leads', 'tasks', 'recurring', 'assistant', 'activity'].includes(section)) setTabValue(section as typeof tab);
  ...
  return <main className="studio-shell"><header className="studio-nav"><button type="button" className="brand-button" onClick={() => setScreen('choose')}><b>✾</b><span>Bramble &amp; Petal<small>{screen === 'business' ? 'FLORIST STUDIO APP' : 'CLIENT PLANNING STUDIO'}</small></span></button>{screen === 'business' ? <nav ref={workspaceNav} aria-label="Business workspace">{nav('dashboard', 'Dashboard')}{nav('inventory', 'Inventory')}{nav('calculator', 'Recipe Builder')}{nav('catalogue', 'Inspiration')}{nav('production', 'Production')}{nav('calendar', 'Calendar')}{nav('clients', 'Clients')}<select aria-label="More Studio tools" value={['leads','suppliers','deliveries','hire','payments','reports','insights','tasks','recurring','assistant','activity','settings','jobs'].includes(tab) ? tab : ''} onChange={e => { if (e.target.value) setTab(e.target.value as typeof tab); }}><option value="">More Studio tools</option>{[['leads','Enquiries'],['suppliers','Suppliers & ordering'],['deliveries','Delivery & setup'],['hire','Hire items'],['payments','Payments'],['reports','Reporting'],['insights','Flower & waste insights'],['tasks','Tasks'],['recurring','Recurring flowers'],['assistant','AI assistant'],['activity','Audit history'],['jobs','Purchased work'],['settings','Settings']].map(([key,label]) => <option value={key} key={key}>{label}</option>)}</select></nav> : <nav ref={workspaceNav} aria-label="Planning type">{(['Wedding', 'Funeral', 'Corporate'] as const).map(type => <button type="button" aria-current={clientType === type ? 'page' : undefined} className={clientType === type ? 'active' : ''} key={type} onClick={() => setClientType(type)}>{type}</button>)}</nav>}<div className="studio-session-actions"><a className="studio-website-link" href="/">View website ↗</a><span className={`studio-save-state ${saveState}`} aria-live="polite">{saveState === 'saving' ? 'Saving…' : saveState === 'error' ? 'Database save issue' : saveState === 'saved' ? 'All changes saved' : 'Connecting…'}</span><button type="button" className="exit-button" onClick={() => setScreen('choose')}>Change space</button><button type="button" className="signout-button" onClick={() => void signOut()}>Sign out</button></div></header>
    ...
    {screen === 'business' && <GlobalSearch data={data} open={openRecord} />}
    ...
    {screen === 'client' ? (
      ...
      {eventDraft ? <><button type="button" className="back-button" onClick={() => { if (canNavigate()) setEventDraft(null); }}>← Back to clients</button><EventWorkspace key={eventDraft.id} data={data} plan={data.plans.find(p => p.id === eventDraft.id) || eventDraft} save={persist} command={runCommand} onRecipe={openRecipe} onDirty={() => { workspaceDirty.current = true; }} /></> : <>
      ...
      {tab === 'dashboard' && <BusinessDashboard data={data} save={persist} open={openRecord} />}
