...
import { saveQuoteToData, winQuote, weddingTotals } from '@/lib/studio-operations';
...
export default function StudioClient() {
  ...
  const [tab, setTabValue] = useState<'dashboard' | 'inventory' | 'calculator' | 'jobs' | 'clients' | 'settings' | 'insights'>('dashboard');
  ...
  const quoteDirty = !!(quote.clientName || quote.lines.length) && JSON.stringify(quote) !== JSON.stringify(data.quotes.find(item => item.id === quote.id));
  ...
  const canNavigate = () => {
    ...
    if ((plannerDirty.current || invoiceDirty.current || buildDirty) && !globalThis.confirm('Leave unsaved changes? Cancel to stay here and save them first.')) return false;
    plannerDirty.current = false; invoiceDirty.current = false;
    return true;
  };
  ...
  const setScreen = (space: typeof screen) => { if (!canNavigate()) return; if (screen === 'client') { setPlanReferences([]); setPlanMaterials([]); setPlanSetupOptions([]); } setScreenValue(space); setSelectedWeddingPlanId(null); setWeddingBuildDraft(null); updateLocation(space); window.scrollTo(0, 0); };
  const setTab = (section: typeof tab) => { if (section === tab || !canNavigate()) return; setTabValue(section); setSelectedWeddingPlanId(null); setWeddingBuildDraft(null); updateLocation(screen, section); window.scrollTo(0, 0); };
  const setClientType = (type: typeof clientType) => { if (type === clientType || !canNavigate()) return; setPlanReferences([]); setPlanMaterials([]); setPlanSetupOptions([]); setClientTypeValue(type); updateLocation(screen, tab, type); };
  ...
  useEffect(() => {
    const restore = () => {
      ...
      if (space === 'business') {
        ...
        if (['dashboard', 'inventory', 'calculator', 'jobs', 'clients', 'settings', 'insights'].includes(section)) setTabValue(section as typeof tab);
  ...
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => { if (saving || quoteDirty || buildDirty || plannerDirty.current || setupDirty) { event.preventDefault(); event.returnValue = ''; } };
  ...
  const saveDefaultMarkup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const settings = { defaultMarkup: n(String(form.get('defaultMarkup'))), defaultWastage: n(String(form.get('defaultWastage'))), labourRate: n(String(form.get('labourRate'))), vatRate: n(String(form.get('vatRate'))) };
  ...
  const deleteCustomer = async (customer: Customer) => {
    ...
    const next: StudioData = {
      ...
      weddingBuilds: data.weddingBuilds.filter(item => item.clientName.toLocaleLowerCase() !== name),
    };
  ...
  const winJob = async () => {
    try {
      const next = winQuote(data, quote);
      if (await persist(addOrUpdateCustomer(next, quote.clientName, quote.contact, quote.notes), `Job won — ${money(totals.grossTotal)} saved and stock deducted.`)) {
        setQuote(blankQuote(data.settings)); setTab('jobs');
      }
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Check the job details.'); }
  };
  ...
  const deleteWonJob = async (jobId: string) => {
    ...
    if (await persist(next, 'Job deleted and its flower stems returned to stock.')) setSelectedJobId(null);
  };
  ...
  const savePlan = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (await persist(addOrUpdateCustomer({ ...data, plans: [plan, ...data.plans] }, plan.clientName, plan.contact, plan.notes), 'Client plan saved to their customer record.')) {
      formElement.reset(); setPlanSetupOptions([]); setPlanReferences([]); setPlanMaterials([]); setPlanMaterialName(''); setPlanMaterialQuantity(1); plannerDirty.current = false;
    }
  };
  ...
  if (screen === 'choose') return <main className="choose-screen"><p className="eyebrow">BRAMBLE &amp; PETAL</p><h1>Studio Hub</h1><p className="welcome-copy">Choose the space that suits this moment.</p><div className="choice-grid"><button type="button" className="choice-card" onClick={() => setScreen('business')}><span>FOR JADE &amp; THE TEAM</span><strong>Business workspace</strong><small>Inventory, quoting, jobs and client plans.</small></button><button type="button" className="choice-card" onClick={() => setScreen('client')}><span>FOR CONSULTATIONS</span><strong>Client planning</strong><small>A customer-safe space for wedding, funeral and corporate planning.</small></button></div><p className={`studio-storage-note ${saveState}`}><span aria-hidden="true">●</span>{saveState === 'saved' ? 'All records up to date' : saveState === 'error' ? 'Connection needs attention' : 'Connecting to the Studio database…'}</p><div className="studio-welcome-links"><a href="/">← View website</a><button type="button" onClick={() => void signOut()}>Sign out</button></div></main>;

  const nav = (name: typeof tab, label: string) => <button type="button" disabled={invoiceBusy || saving} aria-current={tab === name ? 'page' : undefined} className={tab === name ? 'active' : ''} onClick={() => setTab(name)}>{label}</button>;
  ...
  return <main className="studio-shell"><header className="studio-nav"><button type="button" className="brand-button" onClick={() => setScreen('choose')}><b>✾</b><span>Bramble &amp; Petal<small>{screen === 'business' ? 'FLORIST STUDIO APP' : 'CLIENT PLANNING STUDIO'}</small></span></button>{screen === 'business' ? <nav ref={workspaceNav} aria-label="Business workspace">{nav('dashboard', 'Dashboard')}{nav('inventory', 'Inventory')}{nav('calculator', 'Quotes')}{nav('jobs', 'Jobs')}{nav('clients', 'Clients')}{nav('insights', 'Insights')}{nav('settings', 'Settings')}</nav> : <nav ref={workspaceNav} aria-label="Planning type">{(['Wedding', 'Funeral', 'Corporate'] as const).map(type => <button type="button" aria-current={clientType === type ? 'page' : undefined} className={clientType === type ? 'active' : ''} key={type} onClick={() => setClientType(type)}>{type}</button>)}</nav>}<div className="studio-session-actions"><a className="studio-website-link" href="/">View website ↗</a><span className={`studio-save-state ${saveState}`} aria-live="polite">{saveState === 'saving' ? 'Saving…' : saveState === 'error' ? 'Database save issue' : saveState === 'saved' ? 'All changes saved' : 'Connecting…'}</span><button type="button" className="exit-button" onClick={() => setScreen('choose')}>Change space</button><button type="button" className="signout-button" onClick={() => void signOut()}>Sign out</button></div></header>
    ...
    {screen === 'client' ? (
      <section className="studio-content planner" key={clientType}>
        ...
        <section className="records"><h2>Saved {clientType.toLowerCase()} plans</h2>{data.plans.filter(plan => plan.type === clientType).length ? data.plans.filter(plan => plan.type === clientType).map(plan => <article key={plan.id}><b>{plan.clientName} <em>· {plan.status || 'Enquiry'}</em></b><span>{plan.eventDate || 'Date to confirm'} {plan.venue ? `· ${plan.venue}` : ''}</span><p>{plan.arrangements || plan.notes}</p>{plan.references?.length ? <div className="saved-reference-strip">{plan.references.map(reference => <Image key={reference.id} src={reference.dataUrl} alt={reference.caption || reference.name} width={92} height={70} unoptimized />)}</div> : null}{plan.finishedEstimate !== null && <strong>{money(plan.finishedEstimate)}</strong>}{plan.type === 'Wedding' && <button type="button" onClick={() => { setSetupDirty(false); setSetupEditor({ planId: plan.id, client: plan.clientName, options: structuredClone(plan.setupOptions || []), presentation: false }); }}>Flower setups &amp; client proposal</button>}<button type="button" onClick={() => showPlanInvoice(plan)}>Generate invoice</button></article>) : <p>No {clientType.toLowerCase()} plans saved yet.</p>}</section>
      </section>
    ) : <section className="studio-content">
      {tab === 'dashboard' && <>
        <div className="hero-title"><p className="eyebrow">YOUR BUSINESS, AT A GLANCE</p><h1>Studio overview</h1><p>Stock, upcoming plans and the work you’ve made.</p></div><div className="studio-quick-actions"><button className="button" onClick={() => setTab('calculator')}>Create a quote <span>↗</span></button><button onClick={() => setTab('inventory')}>Manage stock <span>↗</span></button><button onClick={() => setScreen('client')}>Plan with a client <span>↗</span></button></div>
        <div className="metric-grid">
          <article><small>INVENTORY VALUE<br />(EX VAT)</small><b>{money(stockValue)}</b></article>
          <article><small>INVENTORY VALUE<br />(INC VAT)</small><b>{money(stockValue * (1 + data.settings.vatRate / 100))}</b></article>
          ...
          <article className="loss-metric"><small>WASTAGE LOSS<br />(AT COST)</small><b>{money(wastageLoss)}</b><span>{wastageStems} stock units logged</span></article>
        </div>
        <div className="dashboard-grid">
          ...
          <article><h2>Upcoming events</h2>{data.plans.some(plan => !plan.eventDate || plan.eventDate >= new Date().toLocaleDateString('en-CA')) ? data.plans.filter(plan => !plan.eventDate || plan.eventDate >= new Date().toLocaleDateString('en-CA')).sort((a, b) => (a.eventDate || '9999').localeCompare(b.eventDate || '9999')).slice(0, 3).map(plan => <p key={plan.id}>{plan.clientName} — {plan.type}<small>{plan.eventDate ? new Date(plan.eventDate + 'T12:00:00').toLocaleDateString('en-GB') : 'Date to confirm'}</small></p>) : <p>No plans added yet.</p>}</article>
      ...
      {tab === 'inventory' && <>
        ...
        <section className="inventory-summary"><div><small>Total value (ex VAT)</small><strong>{money(stockValue)}</strong></div><div><small>VAT ({data.settings.vatRate}%)</small><strong>{money(stockValue * data.settings.vatRate / 100)}</strong></div><div><small>Total value (inc VAT)</small><strong>{money(stockValue * (1 + data.settings.vatRate / 100))}</strong></div></section>
        <details className="studio-invoice-tools" open={invoiceToolsOpen} onToggle={event => setInvoiceToolsOpen(event.currentTarget.open)}><summary>Supplier invoices <span>Upload a delivery or open your invoice library</span></summary><SupplierInvoiceImport onDraftChange={dirty => { invoiceDirty.current = dirty; }} disabled={saving || needsRefresh} onImporting={busy => { invoiceInFlight.current = busy; setInvoiceBusy(busy); }} onImported={(saved, revision) => { setData(normaliseData(saved)); setDatabaseRevision(revision); setSaveState('saved'); setError(''); }} /></details>
        ...
        <section className="inventory-table-wrap"><table className="inventory-table"><thead><tr><th>Flower</th><th>Colour</th><th>Unit cost</th><th>Purchased</th><th>Available</th><th>Actions</th></tr></thead><tbody>{filteredInventory.map(item => <tr key={item.id}><td>{item.name}</td><td>{item.colour || '—'}</td><td>{money(item.costPerStem)}</td><td>{item.stemsPurchased}</td><td><b>{item.stemsRemaining}</b></td><td><div className="inventory-actions"><button type="button" onClick={() => restock(item.id, 10)}>+10</button><button className="wastage-button" type="button" onClick={() => openWastage(item)}>Log wastage</button><button className="delete-button" type="button" onClick={() => deleteInventory(item)}>Delete</button></div></td></tr>)}</tbody></table>{data.inventory.length === 0 ? <p className="empty-state">No flowers yet. Use “Add flower” to start your stock list.</p> : filteredInventory.length === 0 && <p className="empty-state">No stock matches “{inventorySearch}”.</p>}</section>
        {wastageItemId && (() => { const item = data.inventory.find(stock => stock.id === wastageItemId); return item ? <section className="wastage-panel"><div><p className="eyebrow">INVENTORY ADJUSTMENT</p><h2>Log {item.name} as wastage</h2><p>This removes the units from available stock and records the cost as a business loss.</p></div><label>Quantity wasted<input aria-label="Stems wasted" type="number" min="1" max={item.stemsRemaining} value={wastageQuantity} onChange={event => setWastageQuantity(n(event.target.value))} /></label><label>Reason (optional)<input value={wastageReason} placeholder="e.g. damaged, expired, quality issue" onChange={event => setWastageReason(event.target.value)} /></label><div className="wastage-actions"><b>Loss: {money(Math.max(0, Math.min(item.stemsRemaining, Math.floor(wastageQuantity) || 0)) * item.costPerStem)}</b><button type="button" onClick={() => setWastageItemId(null)}>Cancel</button><button className="button" type="button" onClick={recordWastage}>Record wastage</button></div></section> : null; })()}
      </>}
      {tab === 'calculator' && <><p className="eyebrow">PRICING CALCULATOR</p><h1>Build a quote</h1><div className="quote-start-actions"><button type="button" onClick={() => { if (!quoteDirty || globalThis.confirm('Discard this unsaved quote and start another?')) setQuote(blankQuote(data.settings)); }}>+ New quote</button><span>{data.quotes.length} saved quote{data.quotes.length === 1 ? '' : 's'} in your client book</span></div><datalist id="studio-material-picker">{data.materials.map(material => <option key={material.id} value={material.name}>{money(material.unitCost)} / {material.unit}</option>)}</datalist><div className="quote-workspace"><section className="quote-builder"><div className="quote-step"><h2>1. Customer &amp; job</h2><div className="form-columns"><label>Customer name<input value={quote.clientName} placeholder="e.g. Jane Smith" onChange={event => setQuote({ ...quote, clientName: event.target.value })} /></label><label>Contact<input value={quote.contact || ''} placeholder="phone / email" onChange={event => setQuote({ ...quote, contact: event.target.value })} /></label><label>Job type<select value={quote.occasion} onChange={event => setQuote({ ...quote, occasion: event.target.value as Quote['occasion'] })}><option>Bouquet</option><option>Wedding</option><option>Funeral</option><option>Corporate</option></select></label><label>Event / delivery date<input type="date" value={quote.eventDate} onChange={event => setQuote({ ...quote, eventDate: event.target.value })} /></label></div></div><div className="quote-step"><div className="step-title"><h2>2. Flowers used</h2><button onClick={addFlowerLine}>+ Add flower</button></div>{quote.lines.filter(line => line.category === 'stem').map(line => <div className="quote-line-editor" key={line.id}><select aria-label="Flower" value={line.inventoryId || ''} onChange={event => chooseFlower(line.id, event.target.value)}>{data.inventory.map(item => <option key={item.id} value={item.id}>{item.name} — {item.stemsRemaining} stems</option>)}</select><label>Stems used<input aria-label="Stems used" type="number" min="1" value={line.quantity} onChange={event => updateQuoteLine(line.id, { quantity: n(event.target.value) })} /></label><b>{money(line.quantity * line.unitCost)}</b><button aria-label="Remove flower" onClick={() => removeQuoteLine(line.id)}>×</button></div>)}{quote.lines.filter(line => line.category === 'stem').length === 0 && <p className="quote-help">Add flowers from your current inventory.</p>}</div><div className="quote-step"><div className="step-title"><h2>3. Materials &amp; extras</h2><button onClick={addMaterialLine}>+ Add material</button></div>{quote.lines.filter(line => line.category === 'sundry').map(line => <div className="quote-line-editor material-line" key={line.id}><input aria-label="Material name" list="studio-material-picker" value={line.name} placeholder="e.g. ribbon, vase or packaging" onChange={event => { const material = data.materials.find(item => item.name.toLowerCase() === event.target.value.toLowerCase()); updateQuoteLine(line.id, { name: event.target.value, ...(material ? { unitCost: material.unitCost } : {}) }); }} /><label>Quantity<input aria-label="Material quantity" type="number" min="1" value={line.quantity} onChange={event => updateQuoteLine(line.id, { quantity: n(event.target.value) })} /></label><label>Unit cost (£)<input aria-label="Material unit cost" type="number" min="0" step="0.01" value={line.unitCost} onChange={event => updateQuoteLine(line.id, { unitCost: n(event.target.value) })} /></label><button aria-label="Remove material" onClick={() => removeQuoteLine(line.id)}>×</button></div>)}{quote.lines.filter(line => line.category === 'sundry').length === 0 && <p className="quote-help">Add packaging, vases, ribbon or other extras.</p>}</div><div className="quote-step"><h2>4. Pricing</h2><div className="form-columns"><label>Markup %<input type="number" min="0" value={quote.markupPercent} onChange={event => setQuote({ ...quote, markupPercent: n(event.target.value) })} /></label><label>VAT / Tax %<input type="number" min="0" value={quote.vatRate} onChange={event => setQuote({ ...quote, vatRate: n(event.target.value) })} /></label><label>Labour hours<input type="number" min="0" step="0.5" value={quote.labourHours} onChange={event => setQuote({ ...quote, labourHours: n(event.target.value) })} /></label><label>Delivery (£)<input type="number" min="0" value={quote.deliveryFee} onChange={event => setQuote({ ...quote, deliveryFee: n(event.target.value) })} /></label></div><div className="form-columns"><label>Wastage allowance (%)<input type="number" min="0" value={quote.wastagePercent} onChange={event => setQuote({ ...quote, wastagePercent: n(event.target.value) })} /></label><label>Labour rate (£ / hour)<input type="number" min="0" step="0.01" value={quote.labourRate} onChange={event => setQuote({ ...quote, labourRate: n(event.target.value) })} /></label><label>Discount (£)<input type="number" min="0" step="0.01" value={quote.discount} onChange={event => setQuote({ ...quote, discount: n(event.target.value) })} /></label><label className="studio-checkbox"><input type="checkbox" checked={quote.vatApplies} onChange={event => setQuote({ ...quote, vatApplies: event.target.checked })} />Apply VAT</label></div><label>Notes<textarea value={quote.notes || ''} placeholder="Any notes about this order…" onChange={event => setQuote({ ...quote, notes: event.target.value })} /></label></div></section><aside className="quote-summary"><h2>Order summary</h2><span>Flowers cost <b>{money(totals.stemCost)}</b></span><span>Materials cost <b>{money(totals.sundryCost)}</b></span><span>Wastage ({quote.wastagePercent}%) <b>{money(totals.wastage)}</b></span><span>Labour <b>{money(totals.labour)}</b></span><span>Base cost <b>{money(totals.costSubtotal)}</b></span><span>Markup ({quote.markupPercent}%) <b>{money(totals.markup)}</b></span><span>VAT ({quote.vatRate}%) <b>{money(totals.vat)}</b></span><span>Delivery <b>{money(quote.deliveryFee)}</b></span><span>Discount <b>−{money(quote.discount)}</b></span><strong>Total price <b>{money(totals.grossTotal)}</b></strong><button className="button job-won" onClick={winJob} disabled={saving}>Job won — save &amp; deduct stock</button><button className="secondary-action" disabled={saving} onClick={() => void saveQuote()}>{saving ? 'Saving…' : 'Save quote'}</button><p className="quote-help">Saving a quote leaves stock available. Winning the job deducts the quantities above.</p></aside></div></>}
