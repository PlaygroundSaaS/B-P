...
export default function StudioClient() {
  ...
  const addPlanMaterial = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const form = new FormData(event.currentTarget);
    const name = String(form.get('name') || '').trim(); const quantity = Math.max(1, n(String(form.get('quantity') || 1)));
  ...
  const showJobInvoice = (job: StudioData['jobs'][number]) => {
    ...
    const multiplier = lineCost ? job.totals.grossTotal / lineCost : 1;
    setInvoicePreview({ number: `BP-${job.id.slice(0, 8).toUpperCase()}`, clientName: job.clientName, contact: job.contact, occasion: job.occasion, date: job.eventDate || job.wonAt, lines: job.lines.map(line => ({ description: line.name, quantity: line.quantity, total: line.quantity * line.unitCost * multiplier })), subtotal: job.totals.grossTotal - job.totals.vat, vat: job.totals.vat, total: job.totals.grossTotal });
  };
  const showPlanInvoice = (plan: WeddingPlan) => {
    ...
    const materialBase = build.materials.map(material => ({ description: material.name, quantity: material.quantity, total: material.quantity * material.unitCost }));
    const base = [...arrangementBase, ...materialBase].reduce((sum, line) => sum + line.total, 0);
    ...
    setInvoicePreview({ number: `BP-${plan.id.slice(0, 8).toUpperCase()}`, clientName: plan.clientName, contact: plan.contact, occasion: plan.type, date: plan.eventDate || plan.createdAt, lines: [...arrangementBase, ...materialBase].map(line => ({ ...line, total: line.total * multiplier })), subtotal: net, vat, total });
  };
  const savePlan = (event: FormEvent<HTMLFormElement>) => {
    ...
    event.currentTarget.reset(); setPlanReferences([]); setPlanMaterials([]);
  };
    ...
    {screen === 'client' ? (
      <section className="studio-content planner">
        ...
        <form className="planner-form" onSubmit={savePlan}>
          ...
          <section className="plan-material-section"><div className="reference-heading"><div><h2>3. Materials to plan for</h2><p>Record urns, ribbons, baskets, table vases and any other non-floral requirements.</p></div></div><form className="plan-material-form" onSubmit={addPlanMaterial}><input name="name" placeholder="e.g. 12 bud vases" /><input name="quantity" type="number" min="1" defaultValue="1" /><button type="submit">+ Add material</button></form>{planMaterials.length ? <div className="plan-material-list">{planMaterials.map(material => <span key={material.id}><b>{material.quantity} × {material.name}</b><em>{material.status}</em><button type="button" onClick={() => removePlanMaterial(material.id)} aria-label={`Remove ${material.name}`}>×</button></span>)}</div> : <p className="reference-empty">No materials added yet.</p>}</section>
      ...
      {tab === 'clients' && (selectedWeddingPlanId && weddingBuildDraft ? (() => { const plan = data.plans.find(item => item.id === selectedWeddingPlanId); const stockPlan = weddingBuildDraft.inventory.map(flower => { const required = weddingBuildDraft.arrangements.reduce((sum, arrangement) => sum + arrangement.quantity * arrangement.flowers.filter(item => item.inventoryId === flower.id).reduce((lineSum, item) => lineSum + item.stemsPerArrangement, 0), 0); const capacity = flower.stemsPerPurchase * flower.purchases; return { ...flower, required, capacity, leftover: capacity - required, cost: capacity * flower.costPerStem }; }); const flowerCost = stockPlan.reduce((sum, flower) => sum + flower.cost, 0); const materialCost = weddingBuildDraft.materials.reduce((sum, material) => sum + material.quantity * material.unitCost, 0); const baseCost = flowerCost + materialCost; const markup = baseCost * weddingBuildDraft.markupPercent / 100; const net = baseCost + markup; const vat = net * weddingBuildDraft.vatRate / 100; const total = net + vat; return <section className="wedding-builder">
        ...
        <section className="wedding-builder-panel"><div className="builder-heading"><div><p className="eyebrow">3. MATERIAL COSTS &amp; WEDDING PRICE</p><h2>Price the complete wedding</h2><p>Use the materials from the client brief and set costs, markup and VAT.</p></div></div>{weddingBuildDraft.materials.length ? <div className="wedding-material-costs">{weddingBuildDraft.materials.map(material => <div key={material.id}><b>{material.quantity} × {material.name}</b><label>Cost per unit (£)<input type="number" min="0" step="0.01" value={material.unitCost} onChange={event => updateWeddingMaterial(material.id, { unitCost: n(event.target.value) })} /></label><strong>{money(material.quantity * material.unitCost)}</strong></div>)}</div> : <p className="empty-state">No materials were added to the wedding brief. Add them in the planning stage if needed.</p>}<div className="wedding-price-grid"><label>Markup %<input type="number" min="0" value={weddingBuildDraft.markupPercent} onChange={event => setWeddingBuildDraft(current => current ? { ...current, markupPercent: n(event.target.value) } : current)} /></label><label>VAT %<input type="number" min="0" value={weddingBuildDraft.vatRate} onChange={event => setWeddingBuildDraft(current => current ? { ...current, vatRate: n(event.target.value) } : current)} /></label></div><div className="wedding-price-summary"><span>Flower purchases <b>{money(flowerCost)}</b></span><span>Materials <b>{money(materialCost)}</b></span><span>Base cost <b>{money(baseCost)}</b></span><span>Markup ({weddingBuildDraft.markupPercent}%) <b>{money(markup)}</b></span><span>VAT ({weddingBuildDraft.vatRate}%) <b>{money(vat)}</b></span><strong>Wedding price <b>{money(total)}</b></strong></div></section>
