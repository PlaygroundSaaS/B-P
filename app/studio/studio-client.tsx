...
export default function StudioClient() {
  ...
  return <main className="studio-shell">{newClientDraft && <Dialog className="ops-confirm-dialog" label="New client" onClose={() => { if (!newClientDraft.name && !newClientDraft.contact || globalThis.confirm('Discard this new client?')) setNewClientDraft(null); }}><form className="ops-panel" onSubmit={async e => { e.preventDefault(); const customer = { ...newClientDraft, name: newClientDraft.name.trim() }; if (!customer.name) return; if (await persist({ ...data, customers: [customer, ...data.customers] }, 'Client saved.')) { setNewClientDraft(null); setSelectedCustomerId(customer.id); setTabValue('clients'); updateLocation('business','clients'); } }}><h2>A new conversation</h2><Fields value={newClientDraft} onChange={setNewClientDraft} fields={[{key:'name',label:'Client name',required:true},{key:'contact',label:'Phone or email'}]} /><button className="button" disabled={saving}>Save client</button></form></Dialog>}<header className="studio-nav"><button type="button" className="brand-button" onClick={() => setScreen('choose')}><b>✾</b><span>Bramble &amp; Petal<small>{screen === 'business' ? 'FLORIST STUDIO APP' : 'CLIENT PLANNING STUDIO'}</small></span></button>{screen === 'business' ? <nav ref={workspaceNav} aria-label="Business workspace">{nav('dashboard', 'Home')}{nav('clients', 'Clients')}{nav('events', 'Weddings & Events')}{nav('funerals', 'Funerals')}{nav('calculator', 'Recipe Builder')}{nav('inventory', 'Inventory')}<select aria-label="Business Tools" value={['leads','suppliers','deliveries','hire','payments','reports','financial','insights','tasks','recurring','assistant','activity','settings','jobs','catalogue','production','calendar','templates'].includes(tab) ? tab : ''} onChange={e => { if (e.target.value) setTab(e.target.value as typeof tab); }}><option value="">More · Business Tools</option>{[['calendar','Diary & calendar'],['catalogue','Inspiration catalogue'],['production','Production'],['leads','Enquiries'],['suppliers','Suppliers & ordering'],['deliveries','Delivery & setup'],['hire','Hire items'],['payments','Payments'],['financial','Profit, VAT & tax reports'],['reports','Advanced reporting'],['insights','Flower & waste insights'],['tasks','Tasks'],['recurring','Recurring flowers'],['assistant','AI assistant'],['activity','Audit history'],['jobs','Purchased work'],['templates','Recipe library & templates'],['settings','Settings']].map(([key,label]) => <option value={key} key={key}>{label}</option>)}</select></nav> : <nav ref={workspaceNav} aria-label="Planning type">{(['Wedding', 'Funeral', 'Corporate'] as const).map(type => <button type="button" aria-current={clientType === type ? 'page' : undefined} className={clientType === type ? 'active' : ''} key={type} onClick={() => setClientType(type)}>{type}</button>)}</nav>}<div className="studio-session-actions"><a className="studio-website-link" href="/">View website ↗</a><span className={`studio-save-state ${saveState}`} aria-live="polite">{saveState === 'saving' ? 'Saving…' : saveState === 'error' ? 'Database save issue' : saveState === 'saved' ? 'All changes saved' : 'Connecting…'}</span><button type="button" className="exit-button" onClick={() => setScreen('choose')}>Change space</button><button type="button" className="signout-button" onClick={() => void signOut()}>Sign out</button></div></header>
    ...
    {screen === 'client' ? (
      ...
      {tab === 'dashboard' && <StudioHome data={data} open={openRecord} newClient={startClient} newEvent={startEvent} newRecipe={startRecipe} />}
