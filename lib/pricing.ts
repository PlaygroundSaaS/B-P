...
export const DEFAULT_SETTINGS: Settings = { defaultMarkup: 250, defaultWastage: 10, labourRate: 25, vatRate: 20, corporationTaxRate: 25 };
export const emptyStudio = (): StudioData => ({ version: 1, inventory: [], wastage: [], materials: [], customers: [], quotes: [], jobs: [], plans: [], weddingBuilds: [], settings: DEFAULT_SETTINGS });
...
export function calculateTotals(quote: Quote): QuoteTotals {
  ...
  quote = { ...quote, lines: quote.lines.map(line => ({ ...line, quantity: line.quantity * quantity })), labourHours: quote.labourHours * quantity };
  ...
  const extraCost = num(quote.deliveryCost) + num(quote.setupCost) + num(quote.collectionCost) + num(quote.supplierCharges) + num(quote.additionalExpenses);
  const costSubtotal = round2(materialCost + wastage + labour + extraCost);
  const markup = round2(costSubtotal * num(quote.markupPercent) / 100);
  const netTotal = round2(Math.max(0, (quote.sellingPriceExVat ?? (costSubtotal + markup + num(quote.deliveryFee))) - num(quote.discount)));
