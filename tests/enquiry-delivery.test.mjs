...
test('enquiries persist before email; missing or failed email never loses a saved enquiry',async()=>{
 ...
 let allowed=true;const {POST}=compile('../app/api/enquiry/route.ts',{'@/lib/request-body':requestBody,'@/lib/rate-limit':{enquiryAllowed:async()=>allowed},'@/lib/website-enquiry':{captureWebsiteEnquiry:async()=>{saved++;return canSave;}}});
 try{
  ...
  allowed=false;const limited=await POST(request());assert.equal(limited.status,429);assert.equal(saved,before);allowed=true;
  const scripted=new Request('https://www.bramblesandpetals.co.uk/api/enquiry',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(fields)});assert.equal((await POST(scripted)).status,403);assert.equal(saved,before);
